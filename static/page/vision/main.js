...
class Vision {
  constructor (){
    ...
    // Webcam input (ML-style): one shared live stream at a time, owned by a node.
    this.webcamStream = null
    this.webcamVideo = null
    this.webcamNodeId = null
    this._webcamRAF = null
  ...
  init (){
    ...
    if (!this.tree || Object.keys(this.tree).length === 0) {
      let graph = this.makeGraph(undefined, 1)
    ...
    // On navigating back to Vision, handleLink() calls init() BEFORE the section is
    // positioned/animated into view, so the first connection render uses stale board
    // geometry and the links look shifted until the next canvas update. Recompute the
    // connections as soon as the board actually gets laid out / resized.
    if (window.ResizeObserver && this.$.graphBoard && this.$.graphBoard.$) {
      if (this._boardRO)
        this._boardRO.disconnect()
      this._boardRO = new ResizeObserver(() => {
        if (this.inited)
          this.renderConnections()
      })
      this._boardRO.observe(this.$.graphBoard.$)
    }

    // The section ALSO slides in via a CSS transform after init() — that doesn't
    // change the board's size, so the observer above never fires for it. Recompute
    // the connections once the enter-animation settles: on the section's own
    // transitionend, plus timed fallbacks covering the ~0.25s animation.
    let refreshConns = (ev) => {
      if (this.inited && (!ev || (this.section && ev.target === this.section.$)))
        this.renderConnections()
    }
    if (this.section && this.section.$) {
      this.section.$.removeEventListener('transitionend', this._connRefreshFn || (() => {}))
      this._connRefreshFn = refreshConns
      this.section.$.addEventListener('transitionend', refreshConns)
    }
    setTimeout(refreshConns, 80)
    setTimeout(refreshConns, 320)
  }
  ...
  deinit (){
    ...
    this.stopVisionWebcam(false)
    if (this._boardRO) {
      this._boardRO.disconnect()
      this._boardRO = null
    }
    if (this.section && this.section.$ && this._connRefreshFn) {
      this.section.$.removeEventListener('transitionend', this._connRefreshFn)
      this._connRefreshFn = null
    }
    this.nodeDrag = null
    this.connectionDrag = null
    this.addNodeMenuOpen = false
  ...
  makeGraph (sid, graphNumber){
    graphNumber = graphNumber || Object.keys(this.tree || {}).length + 1
    return {
  ...
  normalizeTree (tree){
    let normalized = {}
    let graphIds = Object.keys(tree || {})

    graphIds.forEach((sid, index) => {
      let item = tree[sid] || {}
      let name = typeof item.name === 'string' && item.name.trim() !== ''
        ? item.name.trim()
        : `${visionMsg('VisionGraph', 'Vision graph')} ${index + 1}`
      if (graphIds.length === 1 && name === `${visionMsg('VisionGraph', 'Vision graph')} 2`)
        name = `${visionMsg('VisionGraph', 'Vision graph')} 1`
      normalized[sid] = {
        id:sid,
        name:name,
    ...
    if (Object.keys(normalized).length === 0) {
      let fallback = this.makeGraph(undefined, 1)
...
<path id="vision-drag-preview" display="none" stroke="#facc15" stroke-dasharray="6,3" fill="none" stroke-width="1.8" marker-end="url(#vision-arr-active)"/>`
  ...
  renderNodes (){
    ...
    this.nodes.forEach((node) => {
      ...
      let helpButton = new DOM('button', {
        ...
        title:visionMsg('VisionReadMore', 'Read more about this node')
      })
      helpButton
      ...
      if (node.type === 'input') {
        ...
        uploadLabel = new DOM('label', {
          className:'ghost',
          innerText:visionMsg('MLUploadExamples', 'Upload samples')
        })
      ...
      nodeType.controls.forEach((control) => {
        ...
        if (resolvedControl.type === 'range') {
          ...
          // Editable number field next to the slider for precise fine-tuning.
          // (Was a read-only label.) Wired on 'change' so it doesn't clamp/refresh
          // mid-typing; the slider stays in sync via the value-target machinery.
          let valueInput = new DOM('input', {
            type:'number',
            className:'vision-field-value'
          })
          valueInput.$.value = String(currentValue)
          if (typeof resolvedControl.min != 'undefined') valueInput.$.min = String(resolvedControl.min)
          if (typeof resolvedControl.max != 'undefined') valueInput.$.max = String(resolvedControl.max)
          if (typeof resolvedControl.step != 'undefined') valueInput.$.step = String(resolvedControl.step)
          this.bindNodeControlEvents(valueInput)
          valueInput.onevent('change', this, this.changeNodeParam, [node.id, control.name, resolvedControl.type])

          slider.$.dataset.valueTarget = control.name
          valueInput.$.dataset.valueTarget = control.name
      ...
      if (node.type === 'input') {
        body.append(this._buildInputSourceControl(node, uploadLabel, uploadInput))
      }

      let status = new DOM('div', {
        className:'vision-node-meta vision-node-status',
  ...
  makeContextMenu (){
    ...
    VisionPaletteGroups.forEach((group) => {
      let title = new DOM('button', {
        ...
        type:'button',
        innerText:visionMsg(group.labelKey, group.label)
      })
  ...
  openAddNodeMenu (){
    ...
    VisionPaletteGroups.forEach((group, i) => {
      ...
      tab.textContent = visionMsg(group.labelKey, group.label)
  ...
  uploadImage (nodeId, ev){
    ...
    let node = this.findNode(nodeId)
    if (node)
      node.source = {...(node.source || {}), type:'upload'}
    this.captureInputSource(nodeId, new ImageSource({type:'upload', file}))
  }

  // Store a captured image as the input node's image (shared by all sources).
  _applyInputImage (nodeId, imageData, filename){
  ...
  // Capture one frame from an ImageSource (device/webcam/upload/url) and apply
  // it to the input node. `source` is optional; if omitted it is built from the
  // node's stored source config.
  async captureInputSource (nodeId, source){
    let node = this.findNode(nodeId)
    if (!node)
      return
    source = source || new ImageSource(this.getNodeSource(node))
    let runtime = this.getNodeSource(node)
    this.inputSourceBusy = nodeId
    this.render()
    try {
      let captured = await source.capture()
      let imageData = await captured.toImageData({maxSide:720})
      this._applyInputImage(nodeId, imageData, captured.filename || runtime.url || '')
    } catch (err) {
      console.error('Vision: image source capture failed', err)
      let status = this.$ && this.$.container &&
        this.$.container.$.querySelector(`[data-source-status="${nodeId}"]`)
      if (status)
        status.innerText = (err && err.message) || 'Capture failed.'
    } finally {
      if (source !== undefined && typeof source.stop === 'function' && source.type !== 'webcam')
        source.stop()
      if (this.inputSourceBusy === nodeId)
        this.inputSourceBusy = null
      this.render()
    }
  }

  // The stored image-source config for an input node (defaults to upload).
  getNodeSource (node){
    if (!node.source || typeof node.source !== 'object')
      node.source = {type:'upload'}
    if (!ImageSource || !node.source.type)
      node.source.type = 'upload'
    // Only webcam + upload are supported (like the ML page). Coerce any legacy
    // device/url source so old graphs still load and the picker stays in sync.
    if (node.source.type !== 'webcam' && node.source.type !== 'upload')
      node.source.type = 'webcam'
    return node.source
  }

  setNodeSourceField (nodeId, field, value){
    let node = this.findNode(nodeId)
    if (!node)
      return
    let source = this.getNodeSource(node)
    source[field] = value
    this.saveCurrentGraphState()
    this.render()
  }

  changeInputSourceType (nodeId, ev){
    if (ev.target.value !== 'webcam')
      this.stopVisionWebcam(false)
    this.setNodeSourceField(nodeId, 'type', ev.target.value)
  }
  changeInputSourceDevice (nodeId, ev){ this.setNodeSourceField(nodeId, 'deviceUid', ev.target.value) }
  changeInputSourceUrl (nodeId, ev){ this.setNodeSourceField(nodeId, 'url', ev.target.value) }
  captureInputSourceClick (nodeId){ this.captureInputSource(nodeId) }

  // --- Webcam input, ML-style ------------------------------------------------
  // Start -> a live stream plays inside the input node's preview canvas.
  // Capture sample -> snapshots the current frame and pushes it downstream
  // (reusing the upload capture pipeline). Stop -> closes the camera.
  async startVisionWebcam (nodeId){
    let node = this.findNode(nodeId)
    if (!node)
      return
    this.getNodeSource(node).type = 'webcam'
    this.stopVisionWebcam(false)                 // drop any existing stream first
    try {
      this.webcamStream = await navigator.mediaDevices.getUserMedia({video:true, audio:false})
    } catch (err) {
      console.error('Vision: webcam start failed', err)
      let status = this.$ && this.$.container &&
        this.$.container.$.querySelector(`[data-source-status="${nodeId}"]`)
      if (status)
        status.innerText = (err && err.message) || 'Webcam failed.'
      return
    }
    if (!this.webcamVideo) {
    ...
    this.webcamNodeId = nodeId
    this.render()
    if (this._webcamRAF)
      cancelAnimationFrame(this._webcamRAF)
    this._webcamTick()
  }

  stopVisionWebcam (doRender){
    if (this._webcamRAF) {
      cancelAnimationFrame(this._webcamRAF)
      this._webcamRAF = null
    }
    ...
    this.webcamNodeId = null
    if (doRender !== false && this.inited)
      this.render()
  }

  async captureVisionWebcamSample (nodeId){
    let v = this.webcamVideo
    if (!v || !v.videoWidth)
      return
    ...
    if (!blob)
      return
    let file = new File([blob], 'webcam.png', {type:'image/png'})
    // reuse the upload path: snapshot behaves exactly like an uploaded frame
    await this.captureInputSource(nodeId, new ImageSource({type:'upload', file}))
  }

  // Draw the live webcam frame onto the owning node's preview canvas each frame.
  _webcamTick (){
    if (!this.webcamStream || !this.webcamVideo || !this.webcamNodeId)
      return
    let v = this.webcamVideo
    let canvas = this.$ && this.$.container &&
      this.$.container.$.querySelector(`[data-preview-node-id="${CSS.escape(this.webcamNodeId)}"]`)
    if (canvas && v.videoWidth) {
      let ctx = canvas.getContext('2d')
      let ratio = Math.min(canvas.width / v.videoWidth, canvas.height / v.videoHeight)
      let w = v.videoWidth * ratio, h = v.videoHeight * ratio
      ctx.fillStyle = '#111'
      ctx.fillRect(0, 0, canvas.width, canvas.height)
      ctx.drawImage(v, (canvas.width - w) / 2, (canvas.height - h) / 2, w, h)
    }
    this._webcamRAF = requestAnimationFrame(() => this._webcamTick())
  }

  // Build the image-source picker for an input node: source-type selector +
  // a contextual control (device list / URL field / upload) + a capture button.
  _buildInputSourceControl (node, uploadLabel, uploadInput){
    // No source dropdown — same as the ML page: two buttons (Start webcam +
    // Upload image). Pressing Start webcam swaps them for Capture sample + Stop
    // webcam, and the live feed plays inside the node's preview canvas.
    let container = new DOM('div', {className:'vision-source-control'})
    let row = new DOM('div', {className:'vision-source-row'})
    let streaming = !!this.webcamStream && this.webcamNodeId === node.id

    if (streaming) {
      let busy = this.inputSourceBusy === node.id
      let captureBtn = new DOM('button', {
        className:'primary',
        innerText: visionMsg('MLCaptureExample', 'Capture sample')
      })
      if (busy)
        captureBtn.$.disabled = true
      this.bindNodeControlEvents(captureBtn)
      captureBtn.onevent('click', this, this.captureVisionWebcamSample, [node.id])
      row.append(captureBtn)

      let stopBtn = new DOM('button', {
        className:'ghost',
        innerText: visionMsg('MLStopWebcam', 'Stop webcam')
      })
      this.bindNodeControlEvents(stopBtn)
      stopBtn.onevent('click', this, this.stopVisionWebcam, [])
      row.append(stopBtn)
    } else {
      let startBtn = new DOM('button', {
        className:'ghost',
        innerText: visionMsg('MLStartWebcam', 'Start webcam')
      })
      this.bindNodeControlEvents(startBtn)
      startBtn.onevent('click', this, this.startVisionWebcam, [node.id])
      row.append(startBtn)
      row.append(uploadLabel)            // the "Upload samples" button
    }

    container.append(row)
    container.append(uploadInput)        // hidden file input the upload label triggers

    let status = new DOM('div', {className:'vision-source-status'})
    status.$.dataset.sourceStatus = node.id
...
const VisionNodeTypes = {
  input:{
    ...
    descriptionKey:'VisionDescInput',
  ...
  grayscale:{
    ...
    descriptionKey:'VisionDescGrayscale',
  ...
  resize:{
    ...
    descriptionKey:'VisionDescResize',
  ...
  brightness:{
    ...
    descriptionKey:'VisionDescBrightness',
  ...
  contrast:{
    ...
    descriptionKey:'VisionDescContrast',
  ...
  convolutionMatrix:{
    ...
    descriptionKey:'VisionDescConvolutionMatrix',
  ...
  blendMatrix:{
    ...
    descriptionKey:'VisionDescBlendMatrix',
  ...
  differenceMatrix:{
    ...
    descriptionKey:'VisionDescDifferenceMatrix',
  ...
  multiplyMatrix:{
    ...
    descriptionKey:'VisionDescMultiplyMatrix',
  ...
  colorMatrix:{
    ...
    descriptionKey:'VisionDescColorMatrix',
  ...
  threshold:{
    ...
    descriptionKey:'VisionDescThreshold',
  ...
  adaptiveThreshold:{
    ...
    descriptionKey:'VisionDescAdaptiveThreshold',
  ...
  dilate:{
    ...
    descriptionKey:'VisionDescDilate',
  ...
  erode:{
    ...
    fallbackLabel:'Erode',
    descriptionKey:'VisionDescErode',
  ...
  close:{
    ...
    fallbackLabel:'Close',
    descriptionKey:'VisionDescClose',
  ...
  edges:{
    ...
    fallbackLabel:'Edges',
    descriptionKey:'VisionDescEdges',
  ...
  contours:{
    ...
    descriptionKey:'VisionDescContours',
  ...
  invert:{
    ...
    descriptionKey:'VisionDescInvert',
  ...
  stateOutput:{
    ...
    descriptionKey:'VisionDescStateOutput',
  ...
  objectCountOutput:{
    ...
    descriptionKey:'VisionDescObjectCountOutput',
  ...
  boundingBoxOutput:{
    ...
    descriptionKey:'VisionDescBoundingBoxOutput',
  ...
  rotationOutput:{
    ...
    descriptionKey:'VisionDescRotationOutput',
  ...
  objectCenterOutput:{
    ...
    descriptionKey:'VisionDescObjectCenterOutput',
...
const VISION_NODE_ICONS = {
  ...
  'grayscale':          '<svg viewBox="0 0 20 20" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"><rect x="3" y="3" width="14" height="14" rx="2"/><path d="M10 3v14"/></svg>',
  ...
  'contrast':           '<svg viewBox="0 0 20 20" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"><circle cx="10" cy="10" r="7"/><path d="M10 3 A7 7 0 0 1 10 17Z" fill="currentColor" opacity="0.5" stroke="none"/></svg>',
  ...
  'convolutionMatrix':  '<svg viewBox="0 0 20 20" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><rect x="2.5" y="2.5" width="4" height="4" rx="0.5"/><rect x="8" y="2.5" width="4" height="4" rx="0.5"/><rect x="13.5" y="2.5" width="4" height="4" rx="0.5"/><rect x="2.5" y="8" width="4" height="4" rx="0.5"/><rect x="8" y="8" width="4" height="4" rx="0.5" fill="currentColor" opacity="0.3"/><rect x="13.5" y="8" width="4" height="4" rx="0.5"/><rect x="2.5" y="13.5" width="4" height="4" rx="0.5"/><rect x="8" y="13.5" width="4" height="4" rx="0.5"/><rect x="13.5" y="13.5" width="4" height="4" rx="0.5"/></svg>',
  'colorMatrix':        '<svg viewBox="0 0 20 20" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><rect x="2.5" y="2.5" width="4" height="4" rx="0.5" fill="currentColor" opacity="0.55"/><rect x="8" y="2.5" width="4" height="4" rx="0.5" fill="currentColor" opacity="0.35"/><rect x="13.5" y="2.5" width="4" height="4" rx="0.5" fill="currentColor" opacity="0.2"/><rect x="2.5" y="8" width="4" height="4" rx="0.5"/><rect x="8" y="8" width="4" height="4" rx="0.5"/><rect x="13.5" y="8" width="4" height="4" rx="0.5"/><rect x="2.5" y="13.5" width="4" height="4" rx="0.5"/><rect x="8" y="13.5" width="4" height="4" rx="0.5"/><rect x="13.5" y="13.5" width="4" height="4" rx="0.5"/></svg>',
  ...
  'differenceMatrix':   '<svg viewBox="0 0 20 20" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="3" width="8" height="8" rx="1" fill="currentColor" opacity="0.3"/><rect x="9" y="9" width="8" height="8" rx="1" fill="currentColor" opacity="0.3"/></svg>',
