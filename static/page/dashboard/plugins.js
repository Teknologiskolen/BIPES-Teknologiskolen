...
// sendEasyMQTT was removed: the EasyMQTT relay is retired, so an EasyMQTT dashboard
// target now no-ops (see the 'EasyMQTT' branch in the output handler below).
...
// Header/footer fields may contain escape sequences typed as text (e.g. "\n").
// Interpret the common ones so they become real control characters instead of a
// literal backslash + letter (which would corrupt the command sent to the REPL).
function dataFlowUnescape (text) {
  return String(text || '')
    .replace(/\\r/g, '\r')
    .replace(/\\n/g, '\n')
    .replace(/\\t/g, '\t')
...
function dataFlowWrapPayload (payload, output = {}) {
  return `${dataFlowUnescape(output.header)}${payload}${dataFlowUnescape(output.footer)}`
}
...
function dataFlowResolvedEnvelope (output = {}, transport = 'auto') {
  ...
  // All transports are the REPL now (serial / WebREPL / BLE REPL), so the
  // default is a plain function call for every device — same device-side
  // handler works regardless of how it's connected.
...
function dataFlowDeviceMessage (topic, payload, output = {}, transport = 'auto', format = 'raw-binary') {
  if (format == 'command') {
    ...
    return {
      consoleCode:code,
      // Send command lines RAW (writeRaw via livePush), not through REPL paste
      // mode. The bipes_runtime message protocol reads newline/CR-delimited lines
      // (e.g. "led_on", "drive,40,40"), so a command must arrive as a clean raw
      // line — paste-mode wrapping would corrupt it. A single raw line also still
      // executes fine on a classic REPL device.
      deviceCode:code,
      deviceRaw:true
    }
  }
...
function sendWidgetOutput (setup, topic, value, options = {}) {
  ...
  if (target == 'EasyMQTT')
    return false  // EasyMQTT relay retired: this target sends nowhere.

  // Runtime device (bipes_runtime over serial UART or Bluetooth): the message
  // protocol reads newline-delimited command lines "topic[,value]" (e.g.
  // "set_led,1", "drive,40,40"), NOT a Python "topic(value)" call. When the active
  // target is a runtime device and no explicit dataflow format is set, send the raw
  // command line so _feed/_dispatch can read it. Same string we'd publish over MQTT
  // — one command, every transport.
  let runtimeUID = (flowOut.targetDevice || setup.targetDevice || 'active')
  runtimeUID = runtimeUID == 'active' ? channel.targetDevice : runtimeUID
  if (runtimeUID && channel.runtimeUids && channel.runtimeUids.has(runtimeUID)) {
    // Runtime device: it ONLY understands newline-delimited command lines
    // "topic[,value]" (e.g. set_led,1 / drive,40,40) — NEVER a Python "topic(value)"
    // call. So override whatever envelope a dataflow picked. If an explicit command
    // template was configured, use it; otherwise build topic[,value]. Same string
    // we'd publish over MQTT — one command, every transport.
    let line
    if (flowOut && flowOut.format === 'command')
      line = String(output.deviceCode || output.consoleCode || '').replace(/[\r\n]+$/, '')
    else {
      let valStr = String(value)
      line = valStr === '' ? String(topic) : `${topic},${valStr}`
    }
    if (line)
      command.dispatch(channel, 'livePush', [`${line}\n`, runtimeUID])
    return true
  }
  ...
  static chart (data, dom) {
    ...
    switch (data.setup.source) {
      case 'Console':
		    ...
		    break
		  case 'EasyMQTT':
		    // EasyMQTT retired: legacy EasyMQTT-source charts render empty (no feed).
		    return {
		      sid:data.sid,
