# umqtt_simple.py — vendored MicroPython MQTT client (umqtt.simple), single file.
# Transfer this to the device (like bipes_runtime.py) so it can connect to the
# broker over WiFi. bipes_runtime imports MQTTClient from here when the packaged
# umqtt isn't installed. Public-domain MQTT 3.1.1 client from micropython-lib.
...
class MQTTClient:
    ...
    def connect(self, clean_session=True):
        ...
        cid = self.client_id.encode() if isinstance(self.client_id, str) else self.client_id
        sz = 10 + 2 + len(cid)
        msg[6] = clean_session << 1
        if self.user is not None:
            u = self.user.encode() if isinstance(self.user, str) else self.user
            p = self.pswd.encode() if isinstance(self.pswd, str) else self.pswd
            sz += 2 + len(u) + 2 + len(p)
            msg[6] |= 0xC0
        if self.keepalive:
