...
// try / except <chosen error>. Unlike try_catch (bare except), this catches a SPECIFIC
// exception so you can e.g. clean up on Ctrl-C (KeyboardInterrupt) without hiding real
// bugs. NB KeyboardInterrupt is NOT an Exception, so "any error" does NOT catch Ctrl-C.
  ...
  init: function() {
    ...
    this.appendStatementInput("EXCEPT").setCheck(null)
        ...
        .appendField(new Blockly.FieldDropdown([
          ["any error", "Exception"],
          ["Ctrl-C (stop)", "KeyboardInterrupt"],
          ["value error", "ValueError"],
          ["type error", "TypeError"],
          ["index / key error", "LookupError"],
          ["OS / I/O error", "OSError"],
          ["out of memory", "MemoryError"],
          ["any incl. Ctrl-C", "BaseException"]
        ]), "EXC")
        .appendField("happens, do");
    this.setPreviousStatement(true, null);
    this.setNextStatement(true, null);
    this.setColour(260);
    this.setTooltip("Run the 'try' blocks; if the chosen error happens, run the handler instead of crashing. Use 'Ctrl-C (stop)' to clean up (e.g. clear the screen) when the program is stopped. 'any error' does NOT catch Ctrl-C.");
    this.setHelpUrl("");
  }
};
