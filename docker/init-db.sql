...
CREATE TABLE IF NOT EXISTS teachers (
  ...
  -- Server-created teachers start FALSE so they're forced to change the initial
  -- password on first login (same as students).
...
DO $$
BEGIN
    -- Forced-first-login flag for teachers (added after the original schema).
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns
                   WHERE table_name='teachers' AND column_name='password_changed') THEN
        ALTER TABLE teachers ADD COLUMN password_changed BOOLEAN DEFAULT FALSE;
    END IF;
