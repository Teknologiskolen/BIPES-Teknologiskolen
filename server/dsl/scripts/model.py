...
class InputSpec:
    ...
    check_type: str | list[str] | None = None
    default_value: Any = None
    is_instance_selector: bool = False
    options: list[tuple[str, str]] = field(default_factory=list)
    pin_mode: str | None = None  # "input" | "output" | "any" — generates Pin(n, Pin.IN/OUT) or Pin(n)
    keyword: bool = False  # True → renders as name=value in generated code (e.g. sck=Pin(n))


@dataclass(slots=True)
class ImportSpec:
    module: str
    names: list[str] = field(default_factory=list)  # empty → "import module"
    ...
    def definition_key(self) -> str:
        if self.names:
            return f"from_{self.module}_{'_'.join(self.names)}"
        return f"import_{self.module}"
...
class BlockSpec:
    ...
    use_direct_imports: bool = False  # True → class ref is ClassName; False → module.ClassName
    output_type: str | None = None   # For value blocks: the primary Blockly output type (e.g. "Color565")
    output_supertypes: list[str] = field(default_factory=list)  # e.g. ["Number"] when Color565 <: Number
