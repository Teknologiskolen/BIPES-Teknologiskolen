...
def _scalar_to_python(node) -> Any:
    if node is None:
        return None
    # The grammar now matches each default as a STRING (custom regexes), so cast by
    # the rule that matched: bool / int / float / str.
    cls = type(node).__name__
    val = node.val
    if cls == "BoolDefault":
    ...
    if cls == "FloatDefault":
        return float(val)
    return val  # StrDefault (already a Python str)
...
class BlockdefParser:
    ...
    def _block_spec(self, fb, category: str, color: int | None, module_name: str, url: str, class_ctx) -> BlockSpec:
        ...
        return BlockSpec(
            ...
            # Default to inline inputs; a block can still opt out with `inline = false`.
            inputs_inline=fb.inline.value if fb.inline is not None else True,
            inputs=inputs,
    ...
    def _build_inputs(self, fb, class_ctx, is_ctor: bool) -> list[InputSpec]:
        ...
        # For multiple-instance classes using key_input, EVERY block needs the ID
        # selector: the constructor stores `instances[id] = Class(...)` and each method
        # calls `instances[id].method(...)`. Without it the `{id}` template placeholder
        # has nothing to fill it (the constructor used to be skipped -> broken output).
        if (
            class_ctx
