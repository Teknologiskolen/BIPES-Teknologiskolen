...
def _ensure_role(role_name):
    if _run_dynsec("getRole", role_name, check=False).returncode == 0:
        return

    _run_dynsec("createRole", role_name)


def _ensure_client(username, password):
    if _run_dynsec("getClient", username, check=False).returncode != 0:
...
def disable_device(username):
    _run_dynsec("disableClient", username, check=False)
