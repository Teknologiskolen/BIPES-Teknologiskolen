...
@bp.route('/projects/shared/<uid>', methods=('GET',))
def project_shared_public(uid):
    """Public, no-auth read of a PUBLICLY-shared project's blocks XML, for
    embedding in lessons (/embed?uid=<uid>). Only projects shared publicly are
    returned — class-only/private projects need a login and are not exposed here.
    """
    if not uid:
        return jsonify({'error': 'uid is required'}), 400
    try:
        db = dbase.get_db(_db)
        sql = dbase._s("""
            SELECT p.name, p.data, p.shared_public
            FROM projects p
            WHERE p.share_uid = %s
        """)
        ...
        name, data, shared_public = row
        if not shared_public:
            return jsonify({'error': 'Project is not shared publicly'}), 403

        xml = ''
        try:
            parsed = json.loads(data) if data else {}
            xml = (parsed.get('blocks') or {}).get('xml', '') or ''
        except Exception:
            xml = ''

        resp = jsonify({'name': name, 'xml': xml})
...
@bp.route('/project/cp', methods=('POST',))
...
@bp.route('/project/rm', methods=('POST',))
@auth.require_teacher()
@security.require_same_origin()
def project_rm():
    obj = request.json or {}
...
@bp.route('/project/w', methods=('POST',))
@auth.require_teacher()
@security.require_same_origin()
def project_w():
    obj = request.json or {}
